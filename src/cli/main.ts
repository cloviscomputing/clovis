...
report.command("balance-sheet").description("Report assets, liabilities, and equity").option("--date <date>", "As-of date, YYYY-MM-DD").requiredOption("--quote <asset>", "Quote asset symbol or id").option("--entity <id>", "Scope to an account group").option("--parent <id>", "Alias for --entity").option("--account <id>", "Restrict to an account or account subtree; may be repeated", collectOption, []).option("--include-pending", "Include pending transactions; current balance-sheet reports include them by default").option("--status <status>", STATUS_FILTER_HELP).option("--hide-zero", "Hide zero-balance rows").addHelpText("after", examples(["clovis report balance-sheet --date 2026-06-30 --quote CAD --parent Personal"])).action((opts) => withLedger(program, (ledger) => {
  const args = { date: opts.date, quote_asset_id: opts.quote, ...(opts.includePending === undefined ? {} : { include_pending: Boolean(opts.includePending) }), status: opts.status, hide_zero: Boolean(opts.hideZero), ...scopeArgs(opts) };
  requireScopeWhenAmbiguous(ledger, args, "report balance-sheet");
  return callTool("balance_sheet", args, ledger);
}));
report.command("net-worth").description("Report net worth").option("--date <date>", "As-of date, YYYY-MM-DD").requiredOption("--quote <asset>", "Quote asset symbol or id").option("--entity <id>", "Scope to an account group").option("--parent <id>", "Alias for --entity").option("--account <id>", "Restrict to an account or account subtree; may be repeated", collectOption, []).option("--include-pending", "Include pending transactions; current net-worth reports include them by default").option("--status <status>", STATUS_FILTER_HELP).addHelpText("after", examples(["clovis report net-worth --quote CAD --parent Personal"])).action((opts) => withLedger(program, (ledger) => {
  const args = { date: opts.date, quote_asset_id: opts.quote, ...(opts.includePending === undefined ? {} : { include_pending: Boolean(opts.includePending) }), status: opts.status, ...scopeArgs(opts) };
